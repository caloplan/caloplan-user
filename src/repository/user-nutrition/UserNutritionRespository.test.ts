...
function makeEntryResponse(overrides: Partial<MetadataEntry> = {}): MetadataEntry {
  return {
    ...
    entityKey: "nut-1",
    data: userNutritionRawData as unknown as Record<string, unknown>,
    tags: [],
    version: 1,
    ownerUserId: 1,
...
describe("UserNutritionRespository", () => {
  describe("CRUD", () => {
    ...
    test("getById 从 entry 元数据取 id/user_id/created_time，返回 snake_case", async () => {
      ...
      const result = await repo.getById("nut-1");

      assert.deepEqual(result, expectedUserNutrition);
      assert.equal(calls.length, 1);
      assert.deepEqual(calls[0]!.args, ["user_nutrition_goal", "nut-1"]);
    });
    ...
    test("update 按 id 更新，data 只存业务字段，保留原 date", async () => {
      const { repo, calls } = makeRepo({
        ...
        update: async () =>
          makeEntryResponse({ data: { ...userNutritionRawData, calorie: 2100 } }),
      });

      const result = await repo.update({ id: "nut-1", calorie: 2100 });

      assert.equal(result.id, "nut-1");
      assert.equal(result.calorie, 2100);
      assert.equal(result.carbon, 250); // 未变更字段保留
      assert.equal(result.protein, 150);
    ...
    test("update 记录不存在时抛 404", async () => {
      ...
      await assert.rejects(
        () => repo.update({ id: "not-exist", calorie: 2100 }),
        /不存在/,
      );
  ...
  describe("listMine", () => {
    test("listMine 无参数时用 ownerUserId 过滤当前用户", async () => {
      ...
      assert.deepEqual(result.items[0], expectedUserNutrition);
      const [params] = calls[0]!.args as [Record<string, unknown>];
      assert.equal(params.typeName, "user_nutrition_goal");
      assert.equal(params.ownerUserId, 1);
    ...
    test("listMine 时间范围内存过滤掉范围外的记录", async () => {
      const outOfRangeEntry: MetadataEntry = makeEntryResponse({
        entityKey: "nut-2",
        data: { ...userNutritionRawData, date: "2026-08-01" },
      });
      const { repo } = makeRepo({
        query: async () => ({
          total: 2,
  ...
  describe("历史版本", () => {
    test("listHistory 调用 entries.listVersions，data 只含业务字段，owner 从 version 取", async () => {
      ...
      const result = await repo.listHistory("nut-1");

      assert.equal(result.total, 1);
      assert.equal(result.items[0]!.version, 2);
      assert.equal(result.items[0]!.data.carbon, 250);
      assert.equal(result.items[0]!.data.user_id, "1");
      assert.equal(result.items[0]!.data.id, "nut-1");
      assert.equal(calls.length, 1);
      assert.equal(calls[0]!.method, "listVersions");
      assert.deepEqual(calls[0]!.args, ["user_nutrition_goal", "nut-1", undefined]);
    });

    test("getByVersion 调用 entries.get 并传入 version 参数", async () => {
      const { repo, calls } = makeRepo({ get: async () => makeEntryResponse() });

      const result = await repo.getByVersion("nut-1", 1);
    ...
    test("rollback 调用 entries.rollback 并返回回滚后的 snake_case 数据", async () => {
      ...
      const result = await repo.rollback("nut-1", 1);
